...
## 4. 用 provider 结构化 session 事件替代证据式推断（可做到精确 rebind）
...
`pi --extension, -e <path>` 支持按会话注入扩展，扩展 API 暴露了 session 切换事件：

```typescript
pi.on("session_before_switch", (event, ctx) => {
  // event.reason             "new" | "resume"
  // event.targetSessionFile  要切换到的会话文件（resume 时给出）
});
pi.on("session_start", (event, ctx) => {
  // event.reason             "startup" | "reload" | "new" | "resume" | "fork"
  // event.previousSessionFile
  ctx.sessionManager.getSessionFile();  // 切换后的权威 session file
  ctx.sessionManager.getSessionId();    // 权威 session id
});
pi.on("session_info_changed", ...)  // /name
pi.on("session_before_fork", ...)   // /fork、/clone
```

**建议方向**

- Agora 在 spawn managed 会话时注入 `pi -e <agora-session-reporter>`，由该扩展通过本地 0600 socket 向 Daemon 上报
  `{reason, targetSessionFile, previousSessionFile, sessionId}`；
- Daemon 收到后直接按目标路径 rebind（`HistoryPath` → native id → rekey → Host metadata → Server route），
  不需要等待用户发消息，也不受输入法/自动补全影响；
- 该路径同时覆盖 `/new`、`/fork`、`/clone`、`--resume` 启动等所有切换来源；
- 扩展必须 best-effort：Daemon 不在、socket 不可用或扩展抛错时静默降级到现有证据式检测，绝不阻断 Agent；
- 同类 provider（Claude Code、OpenCode 等）按 `docs/agent-integration.md` 的约定优先消费结构化事件；
- 保留证据式检测作为 fallback（用户自装 pi、未注入扩展、provider 无事件时）。

**已排除的方向（附实测理由）**

- 读 agent 的 live env（`PI_SESSION_ID`/`PI_SESSION_FILE`）：变量本身权威（resume 后 pi 会更新，子进程可见），
  但外部读不到——`ps -E` 对该进程返回空（初始环境还被 `cleanPiEnv` 清过），Linux `/proc/<pid>/environ` 同样只有 exec-time env；
- 终端标题：pi 只放 `<app> - <sessionName> - <cwd>`，是名字不是 id，未命名会话没有；
- transcript 写入归属 / 屏幕指纹 / OCR：非 root 无法确定写者，属统计性推断，不作为方向。

**验收标准**

- 注入扩展后，`/resume`、`/new`、`/fork` 在用户发消息前即完成 rebind，且不产生重复 rebind；
- 没有 `targetSessionFile`（例如切到全新会话）时行为明确（清空 history binding 或标记为无 history）；
- 扩展缺失/报错/Daemon 离线时，Agent 行为不受影响，且证据式 fallback 仍然工作；
- 增加端到端测试：伪扩展触发切换 → Daemon 完成 rekey → Host metadata 与 Server route 收敛；
- 文档明确每个 provider 走哪条路径（结构化事件 / 证据式 / 人工绑定）。
